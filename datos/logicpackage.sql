...
---------------------------------------------------------------------------------------------------------------
  ...
  procedure prueba (vCursor out cur) is
  begin
    open vCursor for
      select * from tbl_etapa_temp where id = 2;
  end prueba;
  ...
  begin 
    open vCursor for 
      ...
      from tbl_viaje v
       ...
       --left join tbl_practico practico on e.practico_id = practico.id
...
---------------------------------------------------------------------------------------------------------------
  ...
  begin
    open vCursor for
      select nro_etapa, rumbo, velocidad, latitud, longitud, to_char(created_at ,'YYYY-MM-DD HH:MI:SS') created_at from tbl_etapa where viaje_id = vViaje order by created_at desc;
  end hist_rvp;
  ...
  procedure pasar_barco(vViajeId in number, vZonaId in number, vEta in date, vCursor out cur) is
  begin
    ...
    temp := etapa.id;
    insert into tbl_etapa (   VIAJE_ID,       ORIGEN_ID,        ACTUAL_ID,               VELOCIDAD,       HRP,        ETA,          FECHA_SALIDA,        FECHA_LLEGADA,          CANTIDAD_TRIPULANTES,       CANTIDAD_PASAJEROS,         PRACTICO_ID,        CAPITAN_ID,      SENTIDO,        CALADO_PROA,         CALADO_POPA,       CALADO_MAXIMO,       CALADO_INFORMADO,      LATITUD, LONGITUD, KM, RUMBO, ACOMPANANTE_ID, CREATED_AT ) 
    VALUES (                  etapa.viaje_id, etapa.actual_id,  vZonaId,                 etapa.velocidad, etapa.hrp,  vEta,    etapa.fecha_salida,  etapa.fecha_llegada,    etapa.cantidad_tripulantes, etapa.cantidad_pasajeros,    etapa.practico_id, etapa.capitan_id, null,           etapa.calado_proa,   etapa.calado_popa,  etapa.calado_maximo, etapa.calado_informado, etapa.latitud, etapa.longitud, etapa.km, etapa.rumbo, etapa.acompanante_id, sysdate) 
    returning ID, NRO_ETAPA, VIAJE_ID,        ORIGEN_ID,        ACTUAL_ID, DESTINO_ID,  VELOCIDAD,        HRP,        ETA,         FECHA_SALIDA,        FECHA_LLEGADA,          CANTIDAD_TRIPULANTES,       CANTIDAD_PASAJEROS,          PRACTICO_ID,      CAPITAN_ID,       SENTIDO,       CALADO_PROA,         CALADO_POPA,        CALADO_MAXIMO,       CALADO_INFORMADO,       LATITUD, LONGITUD, KM, RUMBO, ACOMPANANTE_ID, CREATED_AT                            
    into etapa ;
...
--    where (cxz.puntodecontrol1 = vZonaId or cxz.puntodecontrol2 = vZonaId) and p.id != vZonaId;
  ...
  begin
    ...
    from tbl_viaje v 
    left join tbl_etapa e on (v.id = e.viaje_id and e.nro_etapa = v.etapa_actual) 
    --left join tbl_practico p on (e.practico_id = p.id)
